...
class Rmail
{
    ...
    * @param string $return_path Return path to use
    */
    public function setReplyTo($return_path)
    {
        $this->headers['Reply-To'] = $return_path;
    }
   ...
   protected function encodeHeader($input, $charset = 'ISO-8859-1')
	...
	protected function base64Header($input, $charset = 'ISO-8859-1')
    {
        
        return '=?' . strtoupper($charset) . '?B?' . base64_encode($input) . '?=';
        ...
        switch ($type) {
            case 'mail':
                ...
                if (!empty($this->headers['Subject'])) {
                    $subject = $this->base64Header($this->headers['Subject'], $this->build_params['head_charset']);
            ...
            case 'smtp':
                ...
                if (!empty($this->headers['Subject'])) {
                    $headers[] = 'Subject: '. $this->base64Header($this->headers['Subject'], $this->build_params['head_charset']);
                    unset($this->headers['Subject']);
                }
